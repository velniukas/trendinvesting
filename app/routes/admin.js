...
// TODO var importer = require('../helpers/importer');
var util = require('../helpers/util');
var templet = require('../helpers/template');
...
module.exports.approveView = function(req, res, next) {
  ...
  User.find({ roles : { $ne : 'user' } } ,function(error, users) {
    ...
    res.render('admin/approve', {
      title: 'TRENDinvesting Beta Approval',
      users: users
    });
  });
};
...
module.exports.approve = function(req, res, next) {  
  ...
  var templetPath = path.resolve('./Samples/emailTemplate/approvalForBeta.html');
  templet.getHtmlTemplate("approvalForBeta", { "name" : user.name }, function(error, htmlText){
    ...
    var mailOptions = {
      ...
      subject: "TRENDinvesting : Notification for beta version release", // Subject line
      text: "", // plaintext body
